...
public class DatabaseManager {
    ...
    public static void initializeDatabase() {
        try (Connection conn = getConnection();
            ...
            if (rs.next() && rs.getInt(1) == 0) {
                stmt.execute("""
                    INSERT INTO users (name, age, work_hours, glasses, eye_disease, vision_acuity) 
                    VALUES ('Иван Иванов', 22, 8, 0, NULL, 'норма')
                    """);
    ...
    private static void fillExtendedExercises(Statement stmt) throws SQLException {
        ...
        if (rs.next() && rs.getInt(1) > 0) {
            System.out.println("   Упражнения уже есть в БД (" + rs.getInt(1) + " шт.)");
            return;
        }

        System.out.println("   Заполняем таблицу упражнений...");
        ...
        System.out.println("   Добавлено " + data.length + " упражнений (ID от 1 до 15)");
    }

    private static void fillQuestions(Statement stmt) throws SQLException {
        String[][] questions = {
                // Симптомы утомления (category: symptoms)
                {"Как часто вы чувствуете сухость или жжение в глазах?", "symptoms", "3", "Никогда,Редко,Иногда,Часто,Постоянно"},
                {"Бывают ли у вас головные боли после работы за компьютером?", "symptoms", "2", "Никогда,Редко,Иногда,Часто,Постоянно"},
                {"Замечаете ли вы «пелену» или размытость изображения к концу дня?", "symptoms", "3", "Никогда,Редко,Иногда,Часто,Постоянно"},
                {"Часто ли краснеют глаза к вечеру?", "symptoms", "2", "Никогда,Редко,Иногда,Часто,Постоянно"},
                {"Бывает ли двоение в глазах при взгляде вдаль?", "symptoms", "3", "Никогда,Редко,Иногда,Часто,Постоянно"},
